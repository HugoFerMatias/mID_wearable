...
namespace crypto
{
    public class Sign1Message : Message
    {
        ...
        {
            if (algorithm != null) AddAttribute(HeaderKeys.Algorithm, algorithm, PROTECTED);
        ...
        public void Sign(Key privKey, CBORObject algorithm = null)
        {
            AddSigner(privKey,algorithm);
            PerformSignature();
        }
                    ...
                    case AlgorithmValuesInt.ECDSA_256:
                        {
                            ...
                            if(p==null) { Console.WriteLine("Values is null"); }
                            ECDomainParameters parameters = new ECDomainParameters(p.Curve, p.G, p.N, p.H);
